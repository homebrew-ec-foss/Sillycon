...
module tt_um_kaleidoscope (
    input  wire [7:0] ui_in,    // using ui_in[0] for dir, ui_in[2:1] for speed
  ...
  // direct control inputs from ui_in switches
  wire dir_reverse = ui_in[0];    // ui_in[0]: 0 makes the pattern move outward, 1 reverses it so it moves inward
  wire [1:0] speed_sel = ui_in[2:1]; // ui_in[1] low bit and ui_in[2] high bit for speed: 00 normal, 01 fast, 10 slow, 11 freeze

  // step value for direction: +1 for forward, -1 for reverse
  wire signed [15:0] dir_step = dir_reverse ? -16'sd1 : 16'sd1;
  ...
  assign twist_amount = (speed_sel == 2'b10) ? frame_count[11:8] : frame_count[9:6];
  ...
  wire _unused_ok = &{ena, ui_in[7:3], uio_in, 1'b0};
